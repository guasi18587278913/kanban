...
// Community Dashboard Tables
export const communityGroup = pgTable(
  'community_group',
  {
    id: text('id').primaryKey(),
    productLine: text('product_line').notNull(), // AI产品出海, B站好物交流, YouTube AI视频
    period: text('period'), // 1期, 2期 (可选，有些产品线没有期数)
    groupNumber: integer('group_number').notNull(), // 1, 2, 3 (群号)
    groupName: text('group_name').notNull(), // 完整群名：深海圈丨AI产品出海1期1群
  ...
  (table) => [
    // Query groups by product line
    index('idx_community_group_product_line').on(table.productLine),
    // Composite: Query groups by product line and period
    index('idx_community_group_product_period').on(
      table.productLine,
      table.period
    ),
  ]
...
export const communityDailyReport = pgTable(
  'community_daily_report',
  {
    ...
    reportDate: timestamp('report_date').notNull(), // 报告日期

    // 7个维度的数据
    messageCount: integer('message_count').notNull(), // 总消息数
  ...
  (table) => [
    // Composite unique: 一个群一天只有一个报告
    index('idx_daily_report_group_date').on(table.groupId, table.reportDate),
    // Query reports by date
    index('idx_daily_report_date').on(table.reportDate),
  ]
);

export const communityStarStudent = pgTable(
  'community_star_student',
  {
    id: text('id').primaryKey(),
    reportId: text('report_id')
      .notNull()
      .references(() => communityDailyReport.id, { onDelete: 'cascade' }),
    studentName: text('student_name').notNull(), // 学员昵称
    type: text('type').notNull(), // 标杆类型：变现标杆、技术标杆、精神标杆等
    achievement: text('achievement').notNull(), // 成就描述
    highlight: text('highlight'), // 高光时刻（原文引用）
    suggestion: text('suggestion'), // 运营建议
    sort: integer('sort').default(0).notNull(),
    createdAt: timestamp('created_at').defaultNow().notNull(),
  },
  (table) => [
    // Query star students by report
    index('idx_star_student_report').on(table.reportId),
  ]
);

export const communityKoc = pgTable(
  'community_koc',
  {
    id: text('id').primaryKey(),
    reportId: text('report_id')
      .notNull()
      .references(() => communityDailyReport.id, { onDelete: 'cascade' }),
    kocName: text('koc_name').notNull(), // KOC昵称
  ...
  (table) => [
    // Query KOCs by report
    index('idx_koc_report').on(table.reportId),
  ]
);

export const communityImportLog = pgTable('community_import_log', {
  id: text('id').primaryKey(),
