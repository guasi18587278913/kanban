...
import { getReportByIdV2 } from '@/actions/community-v2-actions';
import {
  Card,
  CardContent,
  CardDescription,
...
export default function CommunityReportDetailPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      if (!id) return;
      try {
        const result = await getReportByIdV2(id);
        setData(result);
      } catch (error) {
        console.error(error);
      } finally {
      ...
      <div className="grid gap-8 md:grid-cols-3">
        ...
        <div>
            <Card className="h-full">
                <CardHeader>
                    <CardTitle>原始聊天记录</CardTitle>
                </CardHeader>
                <CardContent>
                   <div className="h-[600px] overflow-auto pr-2 text-sm whitespace-pre-wrap">
                        {data.rawContent
                          ? data.rawContent.length > 5000
                            ? `${data.rawContent.slice(0, 5000)}\n\n（已截断，完整记录请到原始日志查看）`
                            : data.rawContent
                          : '暂无原始聊天记录'}
                   </div>
                </CardContent>
