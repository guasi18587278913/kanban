...
export async function getDashboardStatsV2() {
  ...
  // 获取所有问答
  const allQaRecords = await db().select().from(qaRecord);
  ...
  // 按来源 ID 分组问答
  const qaBySource = new Map<string, typeof allQaRecords>();
  for (const q of allQaRecords) {
  ...
  const buildLogKey = (params: { productLine: string; period: string; groupNumber: number; date: Date }) =>
    `${params.productLine}__${params.period || ''}__${params.groupNumber}__${params.date.toISOString().slice(0, 10)}`;
  ...
  const enhancedReports = stats.map((stat: any) => {
    // 找到对应的 rawChatLog ID
    const matchingLog = rawLogMap.get(
      buildLogKey({
        productLine: stat.productLine,
        period: stat.period,
        groupNumber: stat.groupNumber,
        date: stat.statsDate,
      })
    );
    ...
    // 构建 groupName
    const periodLabel = stat.period ? `${stat.period}期` : '';
    const groupName = `${stat.productLine}${periodLabel}${stat.groupNumber}群`;

    let actionItems: any[] = [];
    let questionsFromAction: any[] = [];
    const actionListRaw = stat.actionListVerified || stat.actionList;
    if (actionListRaw) {
      try {
        const parsed = JSON.parse(actionListRaw);
        if (Array.isArray(parsed?.actionItems)) actionItems = parsed.actionItems;
        if (Array.isArray(parsed?.questions)) questionsFromAction = parsed.questions;
      } catch {
        // ignore malformed actionList
      }
    }

    const questions = questionsFromAction.length
      ? questionsFromAction
      : dayQa.map((q: any) => ({
...
export async function getReportByIdV2(id: string) {
  const [stat] = await db()
    .select()
    .from(dailyStats)
  ...
  const [log] = await db()
    ...
    .where(
      and(
        eq(rawChatLog.productLine, stat.productLine),
        eq(rawChatLog.period, stat.period),
        eq(rawChatLog.groupNumber, stat.groupNumber),
        eq(rawChatLog.chatDate, stat.statsDate)
      )
    )
    .limit(1);
  ...
  const [starStudents, kocs, qaList, goodNewsList] = await Promise.all([
    db().select().from(starStudent).where(eq(starStudent.sourceLogId, sourceLogId)),
    db().select().from(kocRecord).where(eq(kocRecord.sourceLogId, sourceLogId)),
    db().select().from(qaRecord).where(eq(qaRecord.sourceLogId, sourceLogId)),
    db().select().from(goodNews).where(eq(goodNews.sourceLogId, sourceLogId)),
  ]);
  ...
  return {
    ...
    period: stat.period,
    groupNumber: stat.groupNumber,
    starStudents: starStudents.map((s: any) => ({
      id: s.id,
      studentName: s.studentName,
      type: s.type,
      achievement: s.achievement,
    ...
    goodNewsParsed: goodNewsList.map((n: any) => ({
      content: n.content,
      author: n.authorName,
      date: n.eventDate.toISOString().split('T')[0],
      group: groupName,
    })),
    questions: qaList.map((q: any) => ({
      content: q.questionContent,
      author: q.askerName,
      answeredBy: q.answererName,
      reply: q.answerContent,
