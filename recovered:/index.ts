...
async function processSingleLog(
  ...
  // 语义分析走 LLM（每条日志必跑）
  if (options.useLlm) {
    console.log('  [LLM] Full semantic analysis enabled');
    try {
      const llmSuccess = await processSingleChatLog(log.id);
      if (llmSuccess) {
        ...
        const [qaCountRow] = await db()
          .select({ count: sql<number>`count(*)` })
          .from(qaRecord)
          .where(eq(qaRecord.sourceLogId, log.id));
        const [goodNewsCountRow] = await db()
          .select({ count: sql<number>`count(*)` })
          .from(goodNews)
          .where(eq(goodNews.sourceLogId, log.id));
        const [kocCountRow] = await db()
          .select({ count: sql<number>`count(*)` })
          .from(kocRecord)
        ...
        const [starCountRow] = await db()
          .select({ count: sql<number>`count(*)` })
          .from(starStudent)
          .where(eq(starStudent.sourceLogId, log.id));

        return {
          success: true,
          written: {
            dailyStats: true,
            goodNews: Number(goodNewsCountRow?.count || 0),
            kocRecords: Number(kocCountRow?.count || 0),
            qaRecords: Number(qaCountRow?.count || 0),
            starStudents: Number(starCountRow?.count || 0),
          },
          stats: {
            messages: daily?.messageCount || 0,
            questions: daily?.questionCount || Number(qaCountRow?.count || 0),
            goodNews: Number(goodNewsCountRow?.count || 0),
            koc: Number(kocCountRow?.count || 0),
          },
        };
      }
      return {
